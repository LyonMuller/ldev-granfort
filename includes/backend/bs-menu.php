	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			if ($this->in_mega_menu && $depth === 0) {
					// Abre apenas a div mega-menu-panel e uma <ul> dentro dela se estiver no primeiro nível de um mega menu
					$output .= "\n$indent<div class='mega-menu-panel'><div class='container'><div class='row jcc'>\n";
			} else if ($depth >= 1) {
					// Evita abrir outra <ul> se já está dentro de um mega menu, mas permite para outros submenus
		  ...
		  $dropdown_title = apply_filters('wp_nav_menu_dropdown_title', '', $this->current_item, $depth, $args);
			
			$output .= "{$n}{$indent}<ul$class_names $labelledby>$dropdown_title{$n}";
		}

		public function end_lvl(&$output, $depth = 0, $args = null) {
			$indent = str_repeat("\t", $depth);
			if ($this->in_mega_menu && $depth === 0) {
					// Encerra a lista de itens do submenu.
					$output .= "$indent</ul>\n";
					
					// Insere o conteúdo personalizado aqui, antes de fechar a div do mega menu.
					$extra_content = apply_filters('wp_nav_menu_mega_menu_extra_content', '', $depth, $args);
					$output .= $extra_content;
					
					// Fecha a div do mega menu.
					$output .= "$indent</div></div></div>\n";
	
					// Reseta o estado de mega menu após fechar o nível do mega menu.
					$this->in_mega_menu = false;
			} else {
					// Para menus não mega, apenas fecha a lista de itens do submenu.
					$output .= "$indent</ul>\n";
			}
		}
		...
		public function end_el(&$output, $item, $depth = 0, $args = null) {
			...
			if ($depth === 0 && $this->in_mega_menu) {
					$this->in_mega_menu = false; // Assegura que o estado seja redefinido após um mega menu
			}
		}
		/**
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			$is_mega_menu = get_field('mega_menu', $item);
		...
		public static function fallback( $args ) {
			...
			$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Adicionar Menu', 'wp-bootstrap-navwalker' ) . '">' . esc_html__( 'Adicionar Menu', 'wp-bootstrap-navwalker' ) . '</a></li>';
			$fallback_output .= '</ul>';
